        ...
        public static ClienteMySqlBL mtdoSeleccionarTodoclientes(String codCliente)
        {
            ...
            using (MySqlConnection conex = new MySqlConnection(numerosFacturas.lm_cadena_conexion_MySQL))
            {
                ...
                catch (Exception ex)
                {
                    conex.Close();
                    conex.Dispose();
                    SeleccionarDatosSybaseDAL.actualizarLogMigracionFactura(numerosFacturas.lm_factura_mysql, numerosFacturas.lm_factura_sybase, "ERROR SELEC CLI", ex.Message +" "+ex.StackTrace);
                   // throw ex;
                }
                finally
                {
        ...
        public static ClienteMySqlBL mtdoSeleccionarTodoclientesPadres(String CCONSIGNA)
        {
            ...
            using (MySqlConnection conex = new MySqlConnection(numerosFacturas.lm_cadena_conexion_MySQL))
            {
                try
                {
                    ...
                    " left outer join tipo_cliente tc on c.tipo_cliente_id = tc.tipo_cliente_id " +
                    " left outer join tipo_identificacion ti on c.tipo_identificacion_id = ti.tipo_identificacion_id " +
                    " where CCONSIGNA = @CCONSIGNA ";
                    cmd = new MySqlCommand(sql, conex);
                    cmd.Parameters.Add("@CCONSIGNA", MySqlDbType.VarChar, 100).Value = CCONSIGNA;
                    MySqlDataReader dr = null;
                    conex.Open();
                    dr = cmd.ExecuteReader();
                    while (dr.Read())
                ...
                catch (Exception ex)
                {
                    conex.Close();
                    conex.Dispose();
                    SeleccionarDatosSybaseDAL.actualizarLogMigracionFactura(numerosFacturas.lm_factura_mysql, numerosFacturas.lm_factura_sybase, "ERROR SELEC CLI PADRE", ex.Message +" "+ex.StackTrace);
