        ...
        public static ProveedoresMysql_BL mtdoSeleccionarTodoproveedores(String codProveedor)
        {
            ...
            using (MySqlConnection conex = new MySqlConnection(numerosFacturas.lm_cadena_conexion_MySQL))
            {
                ...
                catch (Exception ex)
                {
                    conex.Close();
                    conex.Dispose();
                    SeleccionarDatosSybaseDAL.actualizarLogMigracionFactura(numerosFacturas.lm_factura_mysql, numerosFacturas.lm_factura_sybase, "ERROR SELE PROV", ex.Message +" "+ex.StackTrace);
