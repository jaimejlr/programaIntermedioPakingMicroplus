            ...
            using (OdbcConnection connection = new OdbcConnection(SettingsConexion.Default.conexionSybase))
                try
                {
                    String SQL_query = "INSERT INTO encabezadofacturas " +
                         ...
                         " VALUES " +
                         " ('" + obj.codemp + "',  '"+obj.numfac+"','"+obj.codven+"', '"+obj.codalm+"', '"+obj.codcli+ "', getdate()  ,'"+obj.lispre+"', '"+obj.observ+"' ,"+obj.poriva+",       ?       ,     ?         , "+obj.totbas+",      ?         , dateadd(mm, 1,getdate() )  , '" + obj.conpag+"', '"+obj.tipefe+"'    ,0.0000,'X',   NULL,    0.0000, 'X',         " + " NULL,  0.0000,    'X',      NULL,  0.0000,  'X'    ,1     , 0    ,  0.0000,   NULL    ,NULL,    0, '"+ obj.codapu+ "',1.0000, '01',   'IMPORTACION',        getdate()  ,      'P',  '" + obj.nomcli+"' , '"+obj.referen+"', '"+obj.codtrans+"' ,     'S',   0.0000, " + " NULL,  0.0000,   0    , NULL, '"+ obj.rucced + "', '" + obj.dircli + "',   'F0',     NULL,   'X',      0,  NULL      ,NULL   ,  0.0000, 0     , 0.0000 ,  0.0000,     NULL,    NULL      ,    'X',     0.0000,    ?              , 0.0000 ,      0,      0,     0.0000,     " + "   0.0000,             ?,      'D',       null,   null,    getdate() ,      getdate(),   '18'          ,  '" + obj.idcliente + "','06',     NULL,    '"+obj.codveh + "',0.0000,   0.0000,  0.0000, 'N'         ,NULL  ,   NULL    ,      NULL,  NULL,  NULL      ,    NULL,   NULL,  0.0000, 'N'   , " + "  NULL,             NULL            ,NULL                    ,NULL           ,NULL           ,NULL   ,null        ,0.0000  ,null           ,NULL     ,NULL       ,0.0000   ,NULL            ,NULL,       " + " CONVERT( CHAR( 20 ), getdate(), 8 ) ,'001'    ,0.0000     ,null        ,NULL    ,NULL         ,NULL       ,NULL       ,'00'        ,'N'  ,'" + obj.claveaccesofegui+ "'  ,NULL             ,NULL    ,'001001'  ,NULL      ,NULL   ,NULL        ,NULL         ,'"+ obj.idcliguia + "'  , '"+ obj.nomcliguia + "'  )";
                    connection.Open();
        ...
        public static string insertarDetalleFacturaSyBase(DetalleFacturaSyBaseBL obj)
        {
            ...
            using (OdbcConnection connection = new OdbcConnection(SettingsConexion.Default.conexionSybase))
                try
                {
                    ...
                    cmd.Parameters.Add("excen", OdbcType.Decimal).Value = obj.excen;
                    cmd.Parameters.Add("codcen", OdbcType.NVarChar).Value = obj.cantid.ToString();
