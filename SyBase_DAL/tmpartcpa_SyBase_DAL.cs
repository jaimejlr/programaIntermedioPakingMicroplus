        ...
        public static string insertartmpartcpaSyBase(tmpartcpa_SyBase_BL obj)
        {
            ...
            using (OdbcConnection connection = new OdbcConnection(SettingsConexion.Default.conexionSybase))
                ...
                catch (OdbcException ex)
                {
                    connection.Close();
                    connection.Dispose();
                    SeleccionarDatosSybaseDAL.actualizarLogMigracionFactura(numerosFacturas.lm_factura_mysql, numerosFacturas.lm_factura_sybase, "ERROR INS tmpartcpa ", ex.Message +" "+ex.StackTrace);
                ...
                catch (Exception ex)
                {
                    connection.Close();
                    connection.Dispose();
                    SeleccionarDatosSybaseDAL.actualizarLogMigracionFactura(numerosFacturas.lm_factura_mysql, numerosFacturas.lm_factura_sybase, "ERROR INS VEND", ex.Message +" "+ex.StackTrace);
                    // throw new Exception(ex.Message);
                }
                finally
                {
